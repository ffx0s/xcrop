...
export default {
  ...
  scaleTo (point, scale) {
    ...
    const scaleChanged = (scale / that.position.scale)
    const { x, y } = calculate(that.position, that.firstOrigin, point, scale / scaleChanged, scaleChanged)

    that.setData({ scale })
    that.moveTo(x, y)
  },

  animate (scale, xpos, ypos) {
    const that = this
